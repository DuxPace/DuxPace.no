...
export function LineReveal({ className = "", direction = "horizontal", delay = 0, duration = 0.8 }: LineRevealProps) {
  const { ref, isInView } = useScrollReveal()
...
export function ImageFade({ src, alt, className = "", delay = 0 }: ImageFadeProps) {
  const { ref, isInView } = useScrollReveal()
    ...
    <span ref={ref} className={className}>
      {words.map((word, index) => (
        <motion.span
          ...
          transition={{ duration: 0.4, delay: delay + index * 0.05, ease: [0.25, 0.1, 0.25, 1] }}
        >
          {word}
        </motion.span>
      ))}
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        animate={isInView ? { opacity: 1, scale: 1 } : {}}
        transition={{ duration: 0.5, delay: delay + 0.2, ease: [0.25, 0.1, 0.25, 1] }}
      />
    </motion.div>
  )
}
