...
export default function InteractiveGlobe() {
  ...
  const handleMouseLeave = useCallback(() => {
    setIsHovering(false)
    if (globeRef.current) globeRef.current.controls().autoRotateSpeed = rotationSpeed
  }, [rotationSpeed])

  const handleClick = useCallback(() => {
    if (globeRef.current) {
      const current = globeRef.current.controls().autoRotateSpeed
      globeRef.current.controls().autoRotateSpeed = current > 0 ? 0 : rotationSpeed
    }
  }, [rotationSpeed])

  const markerTooltip = (marker: object) => {
    const m = marker as Location
    return `<div style="background:rgba(0,0,0,0.95);border:1px solid rgba(255,255,255,0.2);padding:10px 14px;border-radius:6px;font-family:system-ui,sans-serif;font-size:12px;color:white;box-shadow:0 4px 20px rgba(0,0,0,0.5)"><div style="color:#60a5fa;font-weight:600;margin-bottom:4px;font-size:13px">${m.name}</div><div style="color:rgba(255,255,255,0.5);font-family:monospace;font-size:10px">${m.lat.toFixed(2)}°, ${m.lng.toFixed(2)}°</div></div>`
  }
