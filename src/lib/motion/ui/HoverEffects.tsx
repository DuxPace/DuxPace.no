  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        animate={{ borderColor: isHovered ? "rgba(59, 130, 246, 0.5)" : "transparent" }}
        transition={{ duration: 0.3 }}
      />
      <motion.div
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{
          background:
            variant === "primary" ? "#3b82f6" : variant === "secondary" ? "#1e40af" : "white",
        }}
      />
      <motion.span
        className="relative z-10"
        animate={{ color: isHovered && variant === "outline" ? "black" : "inherit" }}
        transition={{ duration: 0.3 }}
      >
        {children}
      </motion.span>
...
export function ColorShiftText({ children, className = "" }: ColorShiftTextProps) {
  ...
  return (
    <motion.span
      ...
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      animate={{ color: isHovered ? "#60a5fa" : "inherit" }}
      transition={{ duration: 0.3 }}
      whileHover={{ scale: 1.05 }}
    >
      {children}
