...
export function SwoopOut({ children, className = "", direction = "right" }: SwoopOutProps) {
  const ref = useRef<HTMLDivElement>(null)
  ...
  return (
    <motion.div
      ...
      animate={
        !isInView
          ? { opacity: 0, ...directions[direction], scale: 0.8, rotate: direction === "left" ? -15 : 15 }
...
export function Floating({ children, className = "", amplitude = 20, duration = 4 }: FloatingProps) {
  return (
    <motion.div
      className={className}
...
export function ElasticSnap({ children, className = "", delay = 0 }: ElasticSnapProps) {
  const ref = useRef<HTMLDivElement>(null)
  const isInView = useInView(ref, { once: true, amount: 0.3 })
...
export function ScrollRotate({ children, className = "", rotate = 360 }: ScrollRotateProps) {
  const ref = useRef<HTMLDivElement>(null)
...
export function MaskReveal({ children, className = "", direction = "left" }: MaskRevealProps) {
  const ref = useRef<HTMLDivElement>(null)
  const isInView = useInView(ref, { once: true, amount: 0.3 })
